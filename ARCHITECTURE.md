...
| `packages/orchestration` | The platform brain: `compile.ts` (YAML → validated compiled JSON), `expression.ts` (the deliberately non-Turing-complete `${...}` language), `engine/engine.ts` (the run loop), `queue.ts` (pg-boss factory + the best-effort post-commit send), `engine/bus.ts` + `redis-bus.ts` (live-event fan-out), `resolve.ts` (submit-time param validation + model-role resolution), `submit.ts` + `run-plan.ts` (the one submission path, whatever triggered it), `usage.ts` (quota gate + reporting), `worker-executors.ts` (which executors are live), `authority.ts` (may this owner still submit — checked at every firing), `schedule-fire.ts` + `trigger-fire.ts` (unattended entry points), `notifications.ts`, `audit.ts`, `artifact-store.ts`, `remote/` (daemon transport + routing), `seed-builtins.ts`. |
| `packages/executor-core` | The `Executor` interface and normalized event stream (ADR-0005), `UsageMeter`, and `FakeExecutor` — the compliance fixture every executor must satisfy. |
...
| `apps/api` | Hono: better-auth + `agr_` API keys, RBAC middleware, registries, template publishing, run lifecycle, approvals, artifacts, SSE, audit log, and the management surfaces for schedules, webhook triggers and keys. Submission itself lives in `@agrippa/orchestration` (the worker fires schedules and triggers, so it cannot live here). Serves the built SPA in production. Never imports executors. |
...
**Crash-resume.** Unexpected engine errors rethrow → pg-boss retries → the engine resumes: succeeded steps skip, a stale `running` step row is marked failed and re-attempted, and the `UsageMeter` rebuilds from persisted `token_usage` rows keyed per attempt — no double counting. This exact path is exercised by a mid-step crash test.
...
4. **Usage rows are keyed `(run, step, attempt)`** so retries consume tokens again without ever double-counting.
...
11. **Unattended work still names a human, and its authority is re-checked at the point of use**: `tasks.created_by`/`runs.created_by` are NOT NULL, so a schedule or webhook trigger submits as its author, and `checkWorkAuthority` re-tests project membership, project status and task-type availability at every firing rather than freezing them at creation — a standing order can do exactly what its owner could do right now, and never more.
12. **One firing, one run**: every unattended submission writes an idempotency key with the run — `runs.origin_key`, unique where not null, set inside the submission transaction — so a queue that re-delivers a job whose transaction already committed cannot charge the project a second time. Only the human paths omit it, where a second request really is a second intent.
12. **A project provider credential outranks worker env, and slots resolve single-provider**: the executor overlay replaces the wire protocol's whole auth-var family (never merges), and submit-time resolution satisfies all of a slot's scoped roles from one provider — a step's base URL is process-wide, so a mixed slot could never execute (ADR-0013).
