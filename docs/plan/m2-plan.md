...
- [x] Workspace/session affinity: **workspace identity is its own column** (`runs.workspace_key`, inherited by follow-ups) with lifetime as a **retention policy** rather than a finalize side effect — collection groups by key and takes the group's latest expiry, so a follow-up extends its ancestor's workspace by existing. Remote affinity came free from ADR-0017's pin (plus a **runtime feature advertisement** so a daemon too old to honor workspace keys is refused a follow-up rather than silently cloning fresh); the central side declines pre-claim for five minutes and then fails `workspace_lost` honestly, with the per-host queue named as the real fix.
- [x] Follow-up steering runs: `POST /runs/:id/followup` + a **Continue** action on a finished run creates a new run (`kind='followup'`) that inherits its predecessor's workspace, session and configuration **verbatim** and executes a synthetic one-step flow; burst coalescing is queue-level (the unstarted run row is the buffer, CAS on `started_at IS NULL`); **resume became a detection capability** (`none`/`unverified`/`verified`) so a rejected resume buys one re-invocation with an engine-composed context-loss disclosure and an executor that cannot prove it is never handed a session at all
- Verify: ☑ compliance suite through **both transports** — resume honored / rejected-then-disclosed / never-resumed-when-unverified / protocol violation fails closed; a follow-up steers the parent's agent in the parent's workspace and session, a steering message containing template syntax is delivered verbatim, a central follow-up declines while its workspace may be elsewhere and fails `workspace_lost` past the grace; API suite covers coalescing, the two 409s and the audit rows. *(Live laptop-daemon follow-up smoke on the deployed stack still outstanding.)*
