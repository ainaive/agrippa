...
**Aliyun Bailian (DashScope) / Qwen**: no worker env needed — an admin adds a `dashscope` credential in project settings and grants the seeded Qwen models. The provider catalog defaults to the Beijing endpoints (`https://dashscope.aliyuncs.com/apps/anthropic` for the claude executor, `…/compatible-mode/v1` for codex); international workspaces set the credential's base-URL override to their workspace-scoped `*.maas.aliyuncs.com` host. Verify Qwen model rates against the live Model Studio price list — Bailian tiers pricing by input length and the seeds carry entry-tier rates.
