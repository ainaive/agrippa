...
- A router `onResolved` subscription clears a stale Radix body lock (`pointer-events: none` left behind when a navigation unmounts a modal layer mid-exit-animation) — guarded to run only when no layer is still open (`role="dialog"/"alertdialog"/"menu"` portals unmount on close), since the persistent shell means back/forward can resolve with e.g. the user menu legitimately up.
...
8. **Project settings** — vertical section nav: General (rename/description + archive danger zone), members & roles, resource grants (toggle matrix per resource type), repos, providers (per-project model-provider credentials: provider select from `PROVIDER_CATALOG`, write-only key input, optional base-URL override, inline replace-key, remove-with-confirm — the key is never read back), quota; destructive actions confirm.
