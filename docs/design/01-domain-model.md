...
- **Storage**: artifacts ≤ 64 KB are stored `inline` (jsonb/text); larger ones go to a disk-backed store at `storage_ref` (a Docker volume path in M1; the indirection allows S3 later). Checkpoint-driving (interaction) artifacts get a raised inline allowance — `INTERACTION_ARTIFACT_MAX_BYTES`, 2 MiB, sized to dominate any schema-valid payload — because resume re-reads them from the row; an artifact that only exists on disk cannot drive its checkpoint. Patch artifacts have no such allowance: at `git.push` the engine reads a larger-than-inline patch back from the store by `storage_ref` for the evidence comparison.
