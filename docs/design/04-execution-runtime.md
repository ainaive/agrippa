...
- `schedule.fire` — one job per firing of a `task_schedules` row, registered with pg-boss's own cron (`boss.schedule(name, cron, {scheduleId}, {tz, key})`), so the calendar and its DST edges are the queue's problem rather than ours. The row is written first and the calendar registered second, so the worker re-registers every enabled schedule at boot: a crash between the two writes would otherwise leave a schedule that never fires, which is invisible by nature. `key` is the schedule id, making re-registration after an edit a replacement rather than a second calendar.
...
**Scheduled submission** (`fireSchedule`, `packages/orchestration/src/schedule-fire.ts`) turns on one distinction: conditions that will never heal on their own **disable** the schedule and announce it; everything else leaves it enabled and records the error. Permanent — owner lost project access, project archived, task type withdrawn — sets `enabled = false` with a `disabled_reason`, writes an audit row, and emits `schedule.disabled`. Fixable — quota exhausted, a revoked grant, no live worker for the executor set — writes `last_error` and emits `schedule.failed`, and next week's firing tries again. Disabling is deliberately loud rather than a silent skip: a schedule that stops quietly is indistinguishable from one that never existed, which is the worst failure mode available to the one feature whose job is to run unattended.

Authority is checked **at fire time**, not revoked eagerly: the handler applies the same `assertProjectRole(owner, project, "member")` gate the manual submit path applies, so a schedule can do exactly what its owner could do at that moment and never more. Eager revocation would mean hooking every path that can remove authority — membership removal, role downgrade, archive, org-role change — and missing one leaves a schedule firing with authority its owner no longer has. Runs are attributed to the owner because `tasks.created_by`/`runs.created_by` are NOT NULL: an unattended submission still names a human, which is what makes the fire-time check meaningful.
