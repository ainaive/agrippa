...
At `git.push`, the engine first compares the stored patch byte-for-byte with a fresh snapshot, including empty values. A patch over the store's 64 KB inline threshold is read back from the artifact store by its `storage_ref` for that comparison (the stored bytes *are* the approved evidence); if it cannot be read back, the push fails with a distinct `contract_violation` rather than proceeding unverified. The SCM adapter stages again while holding its own operation boundary and returns a typed mismatch if the workspace changed between those checks. Only an exact, nonempty match becomes a single Agrippa-authored `commit-tree` child of the clone base. Its sidecar ref makes retries idempotent: a matching tree/parent reuses the commit; any other ref state fails. The pushed PR therefore represents the approved tree, while any local agent commit graph stays only in the disposable workspace (ADR-0012).
