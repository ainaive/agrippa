...
The catalog says what **can** exist; the `executor_registrations` table says what **does**: workers upsert their registered executor ids at boot and heartbeat them on the sweeper interval, and submission rejects a resolved binding whose executor has no recent registration (`executor_unavailable`) — a codex-less deployment fails at submit with an actionable error instead of exhausting queue retries. An empty live set (fresh deployment) skips the check. With **heterogeneous workers** (one has codex, one doesn't) a run can still land on the wrong worker — registrations are deployment-wide, jobs are not routed per executor. The engine throws a typed `ExecutorUnavailableError` *before any status transition*, and the worker **declines** the job for `queued`/`waiting_approval` runs: it appends a `run.deferred` event (so the timeline shows why the run is waiting), completes the job without burning pg-boss retries, and lets the reconciliation sweepers re-enqueue — the run bounces at most every ~60 s until a capable worker claims it. A `running` run rethrows as before (nothing re-enqueues an unclaimed running run; the execution lease is ADR-0009 future work). Per-executor queues would remove the bouncing entirely and remain future work. Note the related **host-affinity** boundary: workspaces are host-local directories, so a run that resumes on a different worker host re-provisions its workspace from the repo — fine for checkout-deterministic steps, which the restart-safety rule already requires.
...
- **Platform git distrusts the workspace.** Every platform-side git invocation (evidence diffs, the finalizing commit, the credentialed push) runs inside a directory the agent could write — including `.git/**`. So the `git()` wrapper uses the same allow-list env scrub (an agent-installed hook or filter would find no `DATABASE_URL`/`AGRIPPA_SECRET_KEY`/provider keys even if it ran), never loads global/system gitconfig, and neutralizes `core.hooksPath`/`core.fsmonitor` on every call. Repo-local `.git/config` — where filters, textconv drivers, `url.insteadOf` push redirection, and credential helpers live — is snapshotted at provision into a **platform sidecar** (`<runId>.platform`, a sibling the write containment can't reach) and restored before every diff/push, with tampering logged. The clone-base SHA anchoring the patch evidence lives in the same sidecar, so an agent moving `refs/agrippa/base` can't blank the diff. Reviewer steps additionally declare per-step `access: readOnly` (template-level), and the engine refuses to push a workspace that drifted from the stored patch evidence. Residual risk: a shell command escaping a degraded OS sandbox could reach the sidecar — that is the container-layer boundary above.
