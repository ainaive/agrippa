...
**Routing** (`routeRun`, evaluated by the worker before engine construction) decides where a run executes: the persisted `runs.runtime_id` pin is absolute (affinity — the runtime owns the workspace); otherwise the central-only strict exclusions apply (any resolved provider with a stored project credential, any project-policy provider, any authorized MCP server with platform-held auth, any template containing `git.push` until the publication inversion lands — platform secrets never ship to daemons and ADR-0013's "project credential wins" precedence must not silently invert); then central workers are preferred whenever a live one covers the executor set, and only what the platform cannot serve goes to the org's live runtimes, scored by executor coverage and the daemon's advertised env auth per provider. A daemon-routed run gets per-run `EngineDeps` (`remoteEngineDeps`): every slot binds a `RemoteExecutor` proxy carrying the catalog capabilities and the DAEMON's `envAuthProviders`, and the workspace manager becomes the remote variant (`diff` reads the uploaded evidence patch; `isIntact` maps to runtime liveness, so a vanished daemon fails a resume `workspace_lost`).
