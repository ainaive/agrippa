...
**Follow-up steering** (`POST /runs/:id/followup`) continues a finished run rather than resubmitting it. Gated like any new work — project accepts work, quota headroom, member role — plus two of its own: the run must be terminal (`409 run_active`; steering a live run is a different feature with a different failure model) and — for a run whose template declares a workspace at all — that workspace must not *already* be collectable at check time (`409 workspace_expired`, so the common case answers as a 409 rather than as a charged run that fails `workspace_lost` on a worker). Best-effort by construction: the collector can take the directory between that check and the dispatch, and a follow-up that loses that narrow race still fails `workspace_lost` — closing it would mean holding a lock across the whole submission. A run with no workspace is never refused on these grounds, because what it continues is the session, which does not expire with a filesystem. The message lands on the run's thread first, so it is visible whatever the queue does; then, if an unstarted follow-up already exists for that workspace, it **absorbs** the message (`coalesced: true`) via a CAS on `started_at IS NULL` — the run row is the burst buffer, because the API runs multiple replicas and an in-process debounce would coalesce per replica. A message arriving after the follow-up starts becomes the next follow-up rather than joining it. The enqueue is delayed by `AGRIPPA_FOLLOWUP_COALESCE_SECONDS` (default 15) and, like every post-commit send here, not awaited. Not on the API-key allow-list: steering is a human affordance, and the allow-list denies by default. What the follow-up then does with the inherited session is the executor contract's business (see [03](03-executor-abstraction.md) rule 6): a resume the executor cannot prove is never attempted, and one it reports rejected degrades to a disclosed fresh start rather than to silence.
