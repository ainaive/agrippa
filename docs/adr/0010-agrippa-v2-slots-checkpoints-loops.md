...
## Amendment (2026-07-23, review round 3)

The Decision's checkpoint paragraph originally read "`input` and `review-gate` **auto-pass** when their source artifact is absent or empty." That symmetric rule was wrong and was corrected — first in the implementation, then (during PR #5's second review round) rewritten in place in the paragraph above, before this ADR was ever merged. The third review round rightly objected to the in-place rewrite of an ADR, so this amendment puts the change itself on the record: the current text states the **asymmetric** semantics the engine enforces — `input` auto-passes on an absent or valid-empty questions artifact ("nothing to ask" is a designed signal), while `review-gate` auto-passes **only** on a present, schema-valid report with zero findings; an absent report fails the run (a gate never passes without evidence), and a present-but-malformed artifact of either kind fails the producing step. The asymmetry exists because the two absences mean different things: a silent clarifier has nothing to ask, but a silent reviewer has not reviewed.
