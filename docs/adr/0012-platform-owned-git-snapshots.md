...
3. **One verified publish commit.** `git.push` repeats the canonical snapshot atomically against the expected patch. Any difference, including empty-versus-nonempty evidence, returns a typed mismatch and fails the run with `contract_violation`. A matching nonempty tree is published as exactly one Agrippa-authored `commit-tree` child of the clone base, with author/committer dates pinned to the base commit so the snapshot commit SHA is fully deterministic. Retries reuse the sidecar branch ref when its tree and parent match, so a lost response cannot create another commit; the ref write itself passes the expected old value (a compare-and-swap), and the engine's per-run worker claim serializes execution, so even a hypothetical racer reproduces the byte-identical commit rather than a duplicate. Local commits made by agents remain useful checkpoints and review context, but their history is intentionally not preserved in the delivered branch.
...
## Amendment (2026-08-01, ADR-0017 publication inversion)

The sidecar gitdir stops anchoring **publication**. `git.push` now builds the snapshot commit in a pristine temp repository from the approved patch bytes: base objects come from the sidecar for central runs (still pristine, still agent-untouchable — its evidence-construction role is unchanged) or, for daemon-routed runs, from the remote by the **server-pinned** base sha — resolved by `ls-remote` at checkout, recorded on `runs.workspace_ref`, and forced onto the daemon's clone; the daemon's own report of what it checked out is only a cross-check (mismatch → typed publish failure). Trusting a daemon-chosen base would let it pick any other reachable commit and smuggle that commit's contents past the approval, since a patch can apply cleanly on many bases. Retry idempotency is re-derived without the sidecar branch ref: identity, message, tree, parent, and both dates (pinned to the base commit) fix the commit SHA completely, and the push is a remote-ref CAS — an existing branch tip must *be* that deterministic commit (idempotent success) or the publish refuses to overwrite it. Decision 3's "the engine's per-run worker claim serializes execution" is now enforced by the ADR-0017 execution lease rather than assumed.
