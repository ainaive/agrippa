...
## Addendum (2026-07-28, gitcode provider)

- **`gitcode` joins the `pr.open` provider set.** GitCode (gitcode.com) exposes a Gitee-style v5 REST API with GitHub-shaped pull-request endpoints (`POST /repos/{owner}/{repo}/pulls`, `html_url` in the response) and Bearer-token auth, so `openGitcodePr` mirrors the GitHub adapter with a different API base (`api.gitcode.com/api/v5` for gitcode.com; origin-relative `/api/v5` elsewhere, which keeps the loopback test forge pattern working). The `repo_connections.provider` column is plain text — the enum is TypeScript-only — so the new value needs no migration.
- **Duplicate recovery generalizes to any 4xx with lookup.** GitHub answers a duplicate create with 422 and GitLab with 409; GitCode's status is undocumented (GitLab-derived platform, Gitee lineage suggests 400). The GitCode adapter therefore attempts recovery on any 4xx: list open PRs on the base branch and match the head client-side (`head.ref`/`source_branch`, plain or `owner:branch` form) — the v5 list endpoint documents no `head` filter. No match rethrows the original error.
- **HTTPS push credentials are unchanged.** `credentialedUrl`'s `x-access-token` username already serves GitLab (PATs there accept any username); GitCode is expected to behave the same. If a live deployment proves otherwise, the fallback is a provider-aware `oauth2` username in that one function.
