...
- **Phases** rail (left): steps grouped by the template's phases (loop rounds repeated with round chips), each with status, duration, tokens, attempts, and its agent slot; checkpoints show their decision state. Steps can be *skipped* (a condition was false or an optional integration is unavailable) — that's normal, not an error.
- **Usage limits** panel: live meters of tokens against the token limit and elapsed time against the time limit, plus any per-phase caps.
...
- The header shows live **token consumption** and total duration.
...
- **Failures** always carry a reason: `usage_limit_exceeded` (the run's token limit or the project quota), `approval_rejected`, `contract_violation` (a required artifact was never produced), `timed_out`, or a step error. Partial progress — steps, outputs, artifacts produced before the failure — remains visible and downloadable.
