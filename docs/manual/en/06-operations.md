...
|---|---|
| Deploy fails `AGRIPPA_VERSION did not reach compose; images would be mistagged` | Compose could not render the file at all — almost always a required variable is unset, and `POSTGRES_PASSWORD` is the one that became mandatory. Set it in `infra/env/.env` (see `infra/env/.env.example`) and redeploy; your stack was rolled back and is still running, so this is not urgent. The message names tagging because the *previous* release's `deploy.sh` discarded compose's stderr, and script changes only take effect one deploy later. **Do not check by running `compose config` in `/opt/agrippa`** — the rollback already reset the checkout to the previous commit, whose compose file still defaulted the password, so it renders cleanly and tells you nothing. To see compose's real error, render the commit that failed (its SHA is in the deploy's `after a failed deploy of …` line): `cd /opt/agrippa && git show <failed-sha>:infra/docker-compose.yml \| docker compose -p agrippa -f - --env-file infra/env/.env config`. Deploys from this release onward report the cause directly. |
