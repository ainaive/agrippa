...
守护进程注册自己能执行什么——普通的 `codex login`（`~/.codex/auth.json`）即被识别为 openai 凭证，无需环境变量密钥——然后长轮询领取任务、把结果流式上报。只有在没有任何中心 worker 同时覆盖所需执行器**及其服务商凭证**时任务才会路由给它（没有密钥的中心 worker 不会挡住有凭证的守护进程；凭证按执行器分别计分，claude+codex 混合守护进程只需 claude 有 anthropic、codex 有 openai，而不是两者都要全部），并且**绝不**路由任何触及平台持有密钥的任务（已存储的项目服务商凭证、带认证的 MCP 服务）——那些始终留在中心 worker 上，保证项目自己配置的密钥永远优先于守护进程的登录。仓库用**机器自身的 git 凭证**克隆（平台的仓库令牌从不离开服务器）；工作区位于 `~/.agrippa/workspaces`。在管理界面撤销令牌可立即切断该机器；执行中途失联的守护进程会在 2 分钟死信超时后使该步骤失败，随后适用任务正常的重试策略。守护进程的能力边界由服务端约束：它只能上报事件、上传产物（接收时即哈希），无法批准检查点、伪造审计记录、推送代码或消耗其他项目的配额。
